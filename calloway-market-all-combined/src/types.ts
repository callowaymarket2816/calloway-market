export interface Product {
  ...
  price?: number; // DoorDash price (cost x 1.333 markup) from verified inventory. If undefined, the UI must show "Price unavailable" — never guess one.
  storePrice?: number; // Real in-store register price. This is what the website should display, per the store's request to show in-store pricing rather than the marked-up DoorDash price.
  featured?: boolean; // Manually curated by the merchant each month via a toggle in the dashboard. Shown in the "Featured This Month" section on the homepage.
