      ...
      {products.some((p) => p.featured) && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            <span className="text-[13px] font-bold tracking-[0.15em] text-[#C4A484] uppercase">
              ⭐ Featured This Month
            </span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {products.filter((p) => p.featured).map((product) => (
              <div
                ...
                className="bg-[#121110] border border-[#C4A484]/30 hover:border-[#C4A484] p-4 cursor-pointer transition group"
              >
                <p className="font-serif text-sm text-[#F4F1ED] group-hover:text-[#C4A484] transition-colors line-clamp-2 mb-2">
                  {product.name}
                </p>
                <div className="flex items-baseline justify-between">
                  {(product.storePrice ?? product.price) ? (
                    <span className="text-lg font-serif text-[#C4A484]">
                      ${(product.storePrice ?? product.price as number).toFixed(2)}
                    </span>
                  ) : (
                    <span className="text-[10px] text-[#F4F1ED]/40 uppercase">Price unavailable</span>
                  )}
                  <span className={`text-[9px] uppercase tracking-wider px-2 py-0.5 border font-bold ${getStockBadgeColor(product.stockStatus)}`}>
